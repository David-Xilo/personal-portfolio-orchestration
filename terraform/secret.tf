...
resource "random_password" "frontend_auth_secret" {
  length  = 32
  special = true
}
...
resource "google_secret_manager_secret" "frontend_auth_key" {
...
resource "google_secret_manager_secret_version" "frontend_auth_key" {
  secret      = google_secret_manager_secret.frontend_auth_key.id
  secret_data = random_password.frontend_auth_secret.result
}
...
resource "google_secret_manager_secret_iam_member" "cloud_run_frontend_auth_access" {
  secret_id = google_secret_manager_secret.frontend_auth_key.secret_id
  role      = "roles/secretmanager.secretAccessor"
  member    = "serviceAccount:${google_service_account.cloud_run_sa.email}"
}
